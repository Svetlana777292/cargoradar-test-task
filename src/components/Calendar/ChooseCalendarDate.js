...
export const ChooseCalendarDate = (props) => {
  const { type, point,singleDateState,rangeDateState,showSingleDate,showRangeDate,setShowDate, onPressSave, onClose, minDate } = props;
  console.log('ChooseCalendarDate ', singleDateState,rangeDateState,showSingleDate,showRangeDate,point,minDate)
  ...
  const handleChooseSingleDate = (data) => {
    console.log('\x1b[96m5\x1b[0m', 'handleChooseSingleDate data:', data) 
    
    const formatDay = dayjs(data).format('DD.MM.YYYY')
    setShowDate(formatDay,'single')
    setOneDate(data) //calendar format
    // setOneDateFormatted(formatDay)
    // console.log('formatDay', formatDay)
    setDisabled(false)
  }

  const handleChooseRangeDate =(start,end)=> {
    console.log('\x1b[96m5\x1b[0m', 'handleChooseRangeDate start end', start,end)    
    let stRange = dayjs(start).format('DD.MM.YYYY')
    let endRange = dayjs(end).format('DD.MM.YYYY')
    setShowDate([stRange, endRange],'range') 

    //в календарь
    setRangeDate([start,end]) //calendar format
  ...
  const handleSaveDates = () => {
    console.log('\x1b[96m5\x1b[0m', 'handleSaveDates ',oneDate,rangeDate)
    //конвертировать дату тут
    if(isActive===true &&  oneDate!==null&&oneDate!==undefined) {
      console.log('oneDate', oneDate)
      onPressSave(oneDate,'single')

    } else if(isActive===false &&  rangeDate!==null){
      // console.log('rangeDate', rangeDate, 'rangeFormatted',rangeFormatted)
      onPressSave(rangeDate,'range')
    }
    // if(isActive===true &&  oneDate!==null&&oneDate!==undefined) {
    //   console.log('oneDate', oneDate)
    //   const formatDay = dayjs(oneDate).format('DD.MM.YYYY')
    //   onPressSave(formatDay,'single')

    // } else if(isActive===false &&  rangeDate!==null){
    //   console.log('rangeDate', rangeDate, 'rangeFormatted',rangeFormatted)
    //   onPressSave(rangeFormatted,'range')
    // }
    onClose()
  }

  useEffect(()=>{
    console.log('\x1b[96m85\x1b[0m', 'useEffect ---->', singleDateState,rangeDateState,showSingleDate,showRangeDate,)

    // console.log('useEffect', 'ChooseCalendarDate','currentDateState',currentDateState,'singleDateState',singleDateState,'rangeDateState',rangeDateState )
    // let curDateStateFormatted = dayjs(currentDateState).format('DD.MM.YYYY') //дата для отображения
    // console.log('1 CCD curDateStateFormatted', curDateStateFormatted)
    // setCurrentDateStateFormatted(curDateStateFormatted)

    if(singleDateState!==null) {
      // let oneDateStateFormatted = formatDate(singleDateState) //YYYY-MM-DD
      // console.log('2 CCD oneDateStateFormatted', oneDateStateFormatted)
      setOneDate(singleDateState)
      // if(curDateStateFormatted!==singleDateState) {
      //   setOneDate(oneDateStateFormatted)
      //   setOneDateFormatted(singleDateState)
      // } else {
      //   console.log('3 curDateStateFormatted!==singleDateState', curDateStateFormatted!==singleDateState)
      //   setOneDate(oneDateStateFormatted)
      //   setOneDateFormatted(singleDateState)
      // }
    }
    ...
    //   setRangeFormatted(rangeDateState)
    // }
  },[singleDateState,rangeDateState,currentDateState])
      ...
      <ScrollView style={{backgroundColor: 'transparent', }} showsVerticalScrollIndicator={false}>
            ...
            <View style={[mainstyles.rowalCjcSb,styles.containerBtn,{}]}>
              <View style={[mainstyles.rowalCjcSb,styles.containerBtnBr,]}/>
                <TouchableOpacity onPress={()=>setIsActive(true)} style={[styles.radioBtn,mainstyles.rowalCjcSb,isActive?styles.radioBtnActive:null]}>
                  {
                    ...
                    <Text style={[mainstyles.text14R,styles.textDate,{paddingLeft: 15,}]}>{showSingleDate}</Text>
                ...
                <TouchableOpacity onPress={()=>setIsActive(false)} style={[styles.radioBtn,mainstyles.rowalCjcSb,isActive?null:styles.radioBtnActive]}>
                  {
                    showRangeDate!==null&&showRangeDate!==undefined?
                    <Text style={[mainstyles.text14R,styles.textDate,{paddingLeft: 15,}]}>{showRangeDate[0]} - {showRangeDate[1]}</Text>
        ...
        }
        {
          ...
          <DateSinglePicker
            ...
            currDate={oneDate} 
            onPress={handleChooseSingleDate} 
            onClose={onClose} 
          />
          :
          <>
            {
              ...
              <DateRangePicker
                ...
                onSuccess={handleChooseRangeDate}
        ...
        <DefaultBtn title={'Выбрать'} disabled={disabled} onPress={handleSaveDates} customStyle={{alignSelf: 'center', marginVertical: 20}}/>
