...
import { Text, View, StyleSheet, TouchableOpacity, TextInput, Keyboard, KeyboardAvoidingView, ScrollView,Platform, Modal } from 'react-native';
...
import { height, promptCalendarDataEnd, width } from '../../util/helperConst';
import { onEditPoint, savePoint } from '../../store/features/addTenderSlice';
import { findJsonObj, findLargestDate, findSmallestDate, findSmallestDateLocal, findSmallestDateUtc, formatDateMlsToCalendar, formatDateToMls } from '../../util/tools';
...
const CreateEndPointScreen = ({route, navigation}) => {
  console.log('CreateEndPointScreen route', )
  ...
  console.log('\x1b[45m showSingleDate %s\x1b[0m ', showSingleDate,dateMls,singleDate)
  console.log('\x1b[45m showRangeDate %s\x1b[0m ', showRangeDate,rangeDateMls,rangeDate)
    ...
    if(route?.params?.data !==null && route?.params?.data !==undefined) {
      if(route.params.data?.action ==='edit') { 
        ...
        if(item.typeDate === 'single') {
          let convertDate = convertUtcToLocalDate(item.dateMls) //!!проверить что возвращает
      ...
      if(singleDate==null) {
        ...
        if(dataTender.startPoints.length > 0) {
          ...
          console.log('qq curDateStateFormatted', curDateStateFormatted)
          console.log('qq currentDateState', currentDateState)
          setShowSingleDate(curDateStateFormatted) //'DD.MM.YYYY'
          setSingleDate(currentDateState) //calendar format
          const dateUtc = formatToUts(currentDateState)
          setDateMls(dateUtc) //(dateUtc) //utc format
        }
      }
    }
  },[route])
    ...
    >
      ...
      <View style={styles.container}>
        <View style={{paddingTop: safeInsets.top}}>
          <HeaderTitleComponent title={'Точка разгрузки'} onPress={()=>navigation.navigate('CreateTender')} customStyle={{paddingBottom: 0}}/>
        </View>
        ...
        <View style={[styles.btnRow,Platform.OS==='android'?{paddingBottom: safeInsets?.bottom}: {}]}>
          <View style={[styles.bottomBtns,]}>
            <TouchableOpacity style={[styles.itemBtn,]} onPress={handleSavePoint}>
              <Text style={[mainstyles.text11M, {color: THEME.PRIMARY,textAlign: 'center'}]}>Сохранить точку разгрузки</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.itemBtn,styles.itemBtnMid,]} onPress={()=>handleSavePoint('addnewone')}>
              <Text style={[mainstyles.text11M, {color: THEME.PRIMARY,textAlign: 'center'}]}>Создать еще одну точку разгрузки</Text>
            </TouchableOpacity>
...
export default CreateEndPointScreen;