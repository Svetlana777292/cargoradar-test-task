...
import { askResetRoute, height, promptSuccCrRoute,  width } from '../../util/helperConst';
import { onChangeIndexOfPoint, onDeletePoint, onResetRoute, setInfoRoute, } from '../../store/features/addRouteSlice';
...
const CreateRouteScreen = ({route,navigation}) => {
  console.log('\x1b[34m%s %s\x1b[0m \x1b[34m%s', 'screens > CreateRoute > CreateRouteScreen.js', 'route:',route);
  ...
  const onOpenCreatePoint = (point) => {
    if(point==='start') {
      dataTender.startPoints.length === 0 ?  navigation.navigate('CreateRoutePoint',{firstOpen: true, type: point}) : navigation.navigate('CreateRoutePoint',{firstOpen: false,type: point})
    } else if(point==='end') {
      dataTender.endPoints.length === 0 ?  navigation.navigate('CreateRoutePoint',{firstOpen: true, type: point}) : navigation.navigate('CreateRoutePoint',{firstOpen: false,type: point})
  ...
  const handleEditPoint = (item,index,point) => {
    console.log('handleEditPoint: ',item,index,point )
    if(point==='start'){
      navigation.navigate('CreateRoutePoint',{data: {item: item, index: index, type: point, action: 'edit'}})
    } else {
      navigation.navigate('CreateRoutePoint',{data: {item: item, index: index, type: point, action: 'edit'}})
    }
  }
  ...
  const getSizeTenders = async() => {
    // const uid = ''//auth().currentUser.uid
    // console.log('getSizeTenders',)
    // try {
    //   let size  = 0
    ...
    //     // setSizeTenders(querySnapshot.size)
  ...
  const handleCreateRoute = async() => {
    ...
    try {
      ...
      const response = await post('routes',tenderObj)
        ...
        <View style={[styles.container,{}]}>
              ...
              <TouchableOpacity style={[{}]} activeOpacity={1} onPress={()=>Keyboard.dismiss()}>
                ...
                <View style={[styles.btnRow,{width: '100%'}]}>
                  <View style={[styles.qwe,{width: !isShowReset ? '100%' : '80%'}]}>
                    <DefaultBtnOutline title={'Создать маршрут'} disabled={isDisableBtn} onPress={handleCreateRoute} 
                    customStyles={[styles.btnCustomStyle, mainstyles.shadowPr10,{width: '100%',}]}
                    />
                  </View>
                  {isShowReset ?
          ...
          }
          {
            ...
            <View style={[mainstyles.containerModalGgBl,mainstyles.alCjcC,{paddingTop: safeInsets?.top,minHeight: height+safeInsets?.top,zIndex: 999}]}>
              <InfoAskWindow data={findJsonObj(jsonDataPrompt,'askResetRoute',askResetRoute)} onPress={handleResetState} onClose={()=>setIsAskResetVisible(false)}/>
            </View>
            :
            null
          }
          ...
          }
          {
            isSuccessedCreate ?
            <View style={[mainstyles.containerModalGgBl,mainstyles.alCjcC,{paddingTop: safeInsets?.top,minHeight: height+safeInsets?.top,zIndex: 999}]}>
              <PromptComponent data={findJsonObj(jsonDataPrompt,'promptSuccCrRoute',promptSuccCrRoute)} onPress={handleResetState}/>
            </View>
            :null
          }
        </View>
